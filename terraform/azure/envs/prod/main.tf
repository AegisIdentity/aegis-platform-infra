...
# `terraform plan` fails until the real prod cluster-admin group object IDs are supplied
...
variable "admin_group_object_ids" {
  type        = list(string)
  description = "Entra ID group object IDs granted Azure RBAC cluster admin on the prod AKS cluster."
}
